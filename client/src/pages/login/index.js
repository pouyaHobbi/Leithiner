...
// Another
import { useStyles } from './../../components/globalStyles/globalStyle'
import styled from 'styled-components'
import {
	emailValidation,
	passwordValidation,
} from './../../lib/fieldsValidation'
...
export const Login = () => {
	...
	const handleLoginClick = () => {
		if (values.password.length === 0) {
			passwordValidation(values.password, (cb) => {
				if (!cb.isValid) {
					setValidation({
						isValid: cb.isValid,
						message: cb.message,
			...
			emailValidation(values.email, (cb) => {
				if (!cb.isValid) {
					setValidation({
						isValid: cb.isValid,
						message: cb.message,
